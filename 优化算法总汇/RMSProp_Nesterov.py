...
def RMSProp_Nesterov(x, target_data, loop_max=1000,epsilon=1):
    ...
    while count < loop_max:
        count += 1
        gradient = np.zeros(2)
        index = random.sample(range(m), int(np.ceil(m * 0.2))) # 取一小批量数据
        temptheta = theta + alpha*v  # 计算临时梯度跟新
        for i in index:
            diff = (np.dot(temptheta, input_data[i]) - target_data[i]) * input_data[i]
            gradient = gradient + diff
        gradient = gradient/len(index)  # 平均梯度
        r = p*r + (1-p)*np.dot(gradient,gradient)  # 内径平方梯度
        v = alpha * v - (eps/math.sqrt(r)) * gradient  # 在这里进行速度更新
...
if __name__=='__main__':
    ...
    theta = RMSProp_Nesterov(x,target_data)
